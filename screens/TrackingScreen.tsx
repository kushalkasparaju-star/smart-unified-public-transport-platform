  ...
  return (
    <div className="h-full flex flex-col relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-slate-200 z-0">
        <div className="w-full h-full opacity-40 bg-[radial-gradient(#94a3b8_1px,transparent_1px)] [background-size:20px_20px]"></div>
        {/* Mock Routes */}
        <svg className="absolute inset-0 w-full h-full">
          <path d="M 50 400 Q 150 350 200 200 T 350 100" fill="none" stroke="#10b981" strokeWidth="6" strokeLinecap="round" className="opacity-50" />
          <path d="M 10 300 Q 100 250 180 300 T 400 350" fill="none" stroke="#3b82f6" strokeWidth="6" strokeLinecap="round" className="opacity-30" />
        </svg>
        {/* Animated Marker */}
      ...
      <div className="mt-auto relative z-10 p-6">
        <div className="bg-white rounded-[3rem] shadow-[0_-20px_50px_rgba(0,0,0,0.1)] p-8 border border-slate-100">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h2 className="text-2xl font-black text-slate-900 mb-1">Route 42B</h2>
              <p className="text-sm font-medium text-slate-400">Heading to Metro Central</p>
            </div>
            <div className="text-right">
              <p className="text-3xl font-black text-emerald-600">08:12</p>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Arrival ETA</p>
            </div>
          </div>

          <div className="grid grid-cols-3 gap-4 mb-8">
            <div className="p-4 bg-slate-50 rounded-3xl text-center border border-slate-100">
              <div className="text-blue-500 flex justify-center mb-2">{ICON_MAP.Users}</div>
              <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Crowd</p>
              <p className="text-sm font-bold text-slate-800">Low</p>
            </div>
            <div className="p-4 bg-slate-50 rounded-3xl text-center border border-slate-100">
              <div className="text-emerald-500 flex justify-center mb-2">{ICON_MAP.Zap}</div>
              <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Status</p>
              <p className="text-sm font-bold text-slate-800">On-Time</p>
            </div>
            <div className="p-4 bg-slate-50 rounded-3xl text-center border border-slate-100">
              <div className="text-amber-500 flex justify-center mb-2">{ICON_MAP.AlertCircle}</div>
              <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Delay</p>
